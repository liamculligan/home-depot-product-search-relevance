...
#Train multiple k-nearest neighbours algorithms on feature_engineering_set_1_reduced using a custom cross-validation scheme and bag the results
...
#Get maximum and minimum values of each column in train
...
for (i in 1:ncol(train)) {
  ...
  train[, i := normalise(train[, i, with=F], train_max[i], train_min[i]), with=F]
  
  test[, i := normalise(test[, i, with=F], train_max[i], train_min[i]), with=F]
...
for (k_value in k_values) {
  ...
  for (rep in 1:10) {
    ...
    #Train model using the custom cross validitian scheme established above and the feature set normalised
    set.seed(seed)
    Mod = train(x = trainM, y = relevance, method = "knn", trControl = trainControl, tuneGrid = tuneGrid, metric = "RMSE", preProc = "range")
    ...
    TuningResults = rbind(TuningResults, c(k_value, rep, RMSE))
  ...
  Tuning = subset(TuningResults, TuningResults$k == k_value)
  ...
  Results = rbind(Results, c(k_value, TuningRMSE))
...
OOSPreds = subset(OOSPreds, OOSPreds$k == Results$k[1])
...
train_knn[, id := trainID]
#Remove rowIndex
train_knn[, rowIndex := NULL]
...
#Set column names
colnames(train_knn) = c(PredColname, "id")

#Save CSV
write.csv(train_knn, Filename, row.names = F)

#Train the model on the full training set using the best parameters
tuneGrid = expand.grid(k=Results$k[1])
...
for (rep in 1:10) {     
  ...
  #Train the model using the full training set with the feature set normalised
  set.seed(seed)
  Mod = train(x = trainM, y = relevance, method = "knn", trControl = trainControl, tuneGrid = tuneGrid, metric = "RMSE", preProc = "range")
...
test_knn = data.frame(relevance = PredTest, id = testID)

Filename = paste0("Test-V", Version, ".csv")
PredColname = paste0("V", Version)

#Set column names
colnames(test_knn) = c(PredColname, "id")

#Save CSV
write.csv(test_knn, Filename, row.names = F)
